...
class Kint
{
    ...
    public static function trace( $trace = null )
    {
        ...
        self::$_stack['trace'][] = ob_get_clean();
    }
     ...
     * @return void|string
     */
    public static function dump( $data = null, $name = null, $open=false)
    {
        if ( !Kint::enabled() ) return;
        ...
        self::$expandedByDefault = $open;

        if($name == null){
            $name = $names[0];
        }
        $output = self::_dump( $data, $name , $callee, $previousCaller);
        self::$_stack['dump'][] = $output;

        return '';
    }

    protected static function _dump( $var, $name , $callee, $prevCaller )
    {
        kintParser::reset();
        return Kint_Decorators_Rich::decorate(
            kintParser::factory( $var, $name ),
            array($callee, $prevCaller)
        );
    }
    ...
    public static function output(){
        ...
        $output .= Kint_Decorators_Rich::_wrapEnd();
        foreach(self::$_stack['trace'] as $stack){
            $output .= $stack;
        }
        echo $output;
    }
...
Kint::_init();
register_shutdown_function(array('Kint', 'output'));